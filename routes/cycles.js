...
router.put('/:id/quarterly-cycles/:quarter', authMiddleware, requireRole(['hr_admin', 'system_admin']), async (req, res) => {
  try {
    ...
    // Validation: Quarter end date must be before or equal to next quarter start date
    // (Same day is allowed: Q1 ends at end of day, Q2 starts at start of day)
    ...
    if (quarter < 4) {
      ...
      if (nextQuarterResult.rows.length > 0 && nextQuarterResult.rows[0].quarter_start_date) {
        ...
        const nextQuarterStartDate = new Date(nextQuarterStart.getFullYear(), nextQuarterStart.getMonth(), nextQuarterStart.getDate());
        const currentQuarterEndDate = new Date(currentQuarterEnd.getFullYear(), currentQuarterEnd.getMonth(), currentQuarterEnd.getDate());
        
        // Q1 end date must be before or equal to Q2 start date (same day is valid)
        if (currentQuarterEndDate > nextQuarterStartDate) {
          validationErrors.push(`Quarter ${quarter} end date (${finalQuarterEnd}) must be before or equal to Quarter ${quarter + 1} start date (${nextQuarterResult.rows[0].quarter_start_date})`);
        }
      }
    }
    
    // Validation: Current quarter start date must be after or equal to previous quarter end date
    // (Same day is allowed: Q1 ends at end of day, Q2 starts at start of day)
    if (quarter > 1) {
      ...
      if (prevQuarterResult.rows.length > 0 && prevQuarterResult.rows[0].quarter_end_date) {
        ...
        // Compare dates only (ignore time) - allow same day
        const prevQuarterEndDate = new Date(prevQuarterEnd.getFullYear(), prevQuarterEnd.getMonth(), prevQuarterEnd.getDate());
        const currentQuarterStartDate = new Date(currentQuarterStart.getFullYear(), currentQuarterStart.getMonth(), currentQuarterStart.getDate());
        
        // Q2 start date must be after or equal to Q1 end date (same day is valid)
        if (prevQuarterEndDate > currentQuarterStartDate) {
          validationErrors.push(`Quarter ${quarter} start date (${finalQuarterStart}) must be after or equal to Quarter ${quarter - 1} end date (${prevQuarterResult.rows[0].quarter_end_date})`);
        }
      }
    }
